...
uint256_t prho(std::string target_pubkey_hex, int key_range, const int WALKERS, const int DP_BITS) {
    ...
    ECPointJacobian H_OFFSET;
    uint64_t min_arr[4];
    uint256_to_uint64_array(min_arr, min_scalar);
    jacobianScalarMultPhi(&H_OFFSET, preCompG, preCompGphi, min_arr, windowSize);
    if (!jacobianIsInfinity(&H_OFFSET)) { modSubP(H_OFFSET.Y, ZERO, H_OFFSET.Y); }

    ECPointJacobian Q_PRIME = target_affine_jac;
    pointAddJacobian(&Q_PRIME, &Q_PRIME, &H_OFFSET);
    ...
    */
    ...
    for (int i = 0; i < N_STEPS; i++) {
        ...
        if (aff_step_tmp.x[0] & 1ULL) {
            ...
            localStepTable[i].b = uint256_t{0, 0, 0, 0};
        }
    ...
    phmap::parallel_flat_hash_map<uint64_t, std::vector<DPEntry>, MurmurHash3, phmap::priv::hash_default_eq<uint64_t>,
    std::allocator<std::pair<const uint64_t, std::vector<DPEntry>>>, 8, std::mutex > dp_table;
                    ...
                    try {
                        ...
                        if (cl) {
                            ...
                            if (!scalarIsZero(db.limbs)) {
                                ...
                                uint64_t res_delta[4];
                                scalarMul(res_delta, da.limbs, inv_db.limbs);
                                uint256_t delta_256;
                                memcpy(delta_256.limbs, res_delta, 32);
                                uint256_t final_k = mod_add_N(delta_256, min_scalar);

                                unsigned char test_pub[33];
                                generatePublicKey(preCompG, preCompGphi, test_pub, final_k.limbs, windowSize);
                                if (memcmp(test_pub, target_pubkey.data(), 33) == 0) {
                                    memcpy(k.limbs, final_k.limbs, 32);
